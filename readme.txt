...
<p>Run the initial script "sql/initial_script.sql" in the Mysql client</p>
Create the tables Classifieds(with the classified records) and Margin_info(with the margins by ad_type and payment_type)

<p>Run the application and start consuming the messages</p>
One worker starts in the foreground by executing the command:
<b>faust -A classifieds_application worker -l info</b>
Check if the messages have been stored with: select * from Classifieds;

<p>Stored function "margin_func"</p>
...
<p>SQL script that returns the margin values</p>
...
<p>Run the stored procedures "margin_by_ad_type_procedure" and "margin_by_payment_type_procedure"</p>
...
<p>BEST TO HAVE</p>
<p>Make the configuration change by database scripts</p>
<p>Save the margins in two distributed tables for fast recovery and fault tolerance</p>
<p> Still did not manage to make the worker run in an infinite loop.</p>
